...
- [x] 이벤트 결과
    ...
    - [x] 할인 후 예상 결제 금액
        ...
        - [x] 음수 이면 IllegalStateException

    - [x] 이벤트 배지
        - [x] '<12월 이벤트 배지>' 출력
        - [x] 총혜택 금액에 따라 배지 이름 다르게 출력
        - [x] 총혜택 금액 >= 5,000 -> 별
        - [x] 총혜택 금액 >= 10,000 -> 트리
        - [x] 총혜택 금액 >= 20,000 -> 산타
        - [x] 부여 X -> "없음"
...
- [x] 객체 지향 생활 체조
    ...
    - [x] 삼항 연산자 금지
    - [x] 메소드 길이 15라인 상한
    - [x] else, switch-case 금지

- [x] 테스트
    - [x] JUnit5, AssertJ 기능 목록 테스트
    - [x] 도메인 로직 단위 테스트
    - [x] UI 로직 제외
    - [x] 핵심 로직 코드, UI 코드 분리

- [x] 예외
    - [x] 사용자의 잘못된 입력에 `IllegalArgumentException` 던지기
    - [x] `[ERROR] message` 에러메시지 출력
    - [x] 해당 부분을 다시 입력 받기
    - [x] Exception 말고, IllegalArgumentException, IllegalStateException 처럼 명확한 예외 던지기

- [x] 입출력 클래스
    - [x] 입력, 출력 클래스 분리
    - [x] 패캐지, 클래스명, 메소드 시그니처 자유롭게 구현

## 제출 요구 사항

- [x] 원본 repo use template
- [x] private 저장소 설정
- [x] woowa-course collaborator 초대
- [x] main 브랜치에 작업
- [x] 지원 플랫폼에 등록